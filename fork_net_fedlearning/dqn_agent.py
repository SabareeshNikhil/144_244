...
Project for Udacity Nanaodgree in Deep Reinforcement Learning (DRL)
...
class Agent():
    ...
    def __init__(self, state_size, action_size, dqn_type='DQN', replay_memory_size=1e5, batch_size=64, gamma=0.99,
    	learning_rate=1e-3, target_tau=2e-3, update_rate=4, seed=0, using_visual=False):
        ...
        self.using_visual = using_visual
    ...
    def step(self, state, action, reward, next_state, done):
        ...
        if self.t_step == 0:
            ...
            if len(self.memory) > self.batch_size:
                experiences = self.memory.sample(self.using_visual)
    ...
    def download_global_net(self, global_net):
        for network_param, (name, global_param) in zip(self.network.parameters(), global_net.named_parameters()):
            if "shared" in name:
                network_param.data.copy_(global_param.data)